...
The generic `id` column is ambiguous — it appears in students,
enrolments, contacts, and other entities. The server uses an
`ENTITY_ID_PREFIX` lookup to assign the correct prefix based on which
entity the data came from:
...
The full default policy is in `inst/mcp/field_policy.yml` (the canonical
source of truth). Below is the current content — if it drifts, the YAML
...
The full `SERVER_INSTRUCTIONS` string is defined in `inst/mcp/server.R`
(search for `SERVER_INSTRUCTIONS <-`). It is maintained there as the
