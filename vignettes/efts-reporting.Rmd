---
title: "EFTS Pro-Rata Reporting"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{EFTS Pro-Rata Reporting}
...
EFTS (Equivalent Full-Time Student) is the standard funding metric for tertiary
education in New Zealand. selmaR includes `selma_efts_report()` to pro-rate each
component's EFTS across calendar months — replicating SELMA's built-in funding
report.
...
2. For each calendar month, calculate the overlap days between the component's
   date range and the month
3. Monthly EFTS = (overlap_days / total_days) * component_efts

This distributes a component's EFTS across months proportionally to how much
of its duration falls within each month.
...
These are available as package constants: `SELMA_FUNDING_GOVT`, `SELMA_FUNDING_INTL`, etc.
