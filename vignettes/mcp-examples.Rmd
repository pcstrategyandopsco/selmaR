---
title: "MCP Server — Usage Examples"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{MCP Server — Usage Examples}
...
The selmaR MCP server exposes SELMA student management data to Claude via
the Model Context Protocol. It runs as a local JSON-RPC 2.0 stdio process
with 7 defence layers protecting PII.

This vignette shows three common usage scenarios.

## How It Works

You ask a question in plain English. Claude picks the right tool, the MCP
server fetches data from your SELMA instance, strips PII, and returns a
safe result. All data stays local.

```{r, echo = FALSE, eval = TRUE, out.width = "100%", fig.alt = "Architecture diagram showing how a plain-English question flows from you to Claude to the selmaR MCP server to the SELMA API, with 7 PII defence layers in the MCP server"}
knitr::include_graphics("mcp-architecture.svg")
```
...
Note: PII fields (surname, forename, email, DOB, NSN) are not present — the
field policy filters them before they reach Claude. IDs are pseudonymised
(e.g. `S-a3f2c1b9` instead of `1001`).

## Example 2: Intake Queries with Aggregation

**User prompt:** "How many students are enrolled per programme, broken down by
status?"
...
provides `save_chart(html, filename)` which safely writes HTML files to the
designated `output_dir`. This function sanitises the filename (no directory
traversal) and only writes to the output directory.
...
This triggers a warning-level notification and is logged in the audit trail.
...
The session report is generated automatically when the server shuts down and
includes tool call history, entities accessed, blocked attempts, and PII
redaction counts.
