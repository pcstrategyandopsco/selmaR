...
#' collection endpoint. Use this to query endpoints not yet covered by a named
#' function in the package — the SELMA API has over 200 collection endpoints
#' and selmaR does not wrap all of them.
#'
#' The returned function validates `filter` parameter names against the schema
#' registry for the connected API version (sourced from the OpenAPI specs at
#' build time). Unknown parameters emit a `cli_warn()` and are dropped rather
#' than being sent to the API silently.
#'
#' For a one-off query without creating a named function, use [selma_get()]
#' directly.
#'
#' @param entity API endpoint path segment (v3 canonical name), e.g.
#'   `"events"`, `"placements"`, `"documents"`. Consult the SELMA v3 OpenAPI
#'   spec for the full list. For v2 connections the endpoint is resolved via
#'   the version alias table automatically.
#' @return A function with signature: \cr
#'   `function(con = NULL, filter = list(), cache = FALSE,` \cr
#'   `         cache_dir = "selma_cache", cache_hours = 24,` \cr
